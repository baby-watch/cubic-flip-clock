...
以下 GIF 为七种皮肤的界面预览，按正常走时仅翻动秒卡片；顶部保持“北京时间”。棋盘格表示透明区域，不属于应用背景。GIF 由网页原型生成，不代表实机性能。
...
- 右倾后在 0.8 秒内回正：切换原版翻页、机械回弹。快速倾斜的实机手感仍待使用者确认。
- 后方返回操作：退出时钟，回到桌面。
...
当前稳定版本为 **1.1.0**，支持七种皮肤、机械回弹、年月日、星期与农历显示，已完成设备 API 与正式按键回调检查。
